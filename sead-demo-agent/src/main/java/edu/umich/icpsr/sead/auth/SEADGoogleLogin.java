...
public class SEADGoogleLogin {
	...
	public static String getAccessToken() {
		try {
			refresh_token = new String(Files.readAllBytes(Paths.get("/home/sead/refresh.txt")));
		...
		if (refresh_token == null) {
			...
			if (refresh_token != null) {
				...
				try {
					...
					log.error("Could not write /home/sead/refresh.txt: " + e.getMessage());
	...
	private static void initGProps() {
		...
		try {
			gProps = mapper.configure(DeserializationFeature.UNWRAP_ROOT_VALUE, true).readValue(SEADGoogleLogin.class.getClassLoader().getResourceAsStream("sead-google.json"), GoogleProps.class);
		} catch (Exception e) {
